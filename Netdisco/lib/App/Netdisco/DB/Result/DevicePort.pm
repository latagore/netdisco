...
__PACKAGE__->belongs_to( neighbor_alias => 'App::Netdisco::DB::Result::DeviceIp',
  sub {
      my $args = shift;
      return {
          "$args->{foreign_alias}.alias" => { -ident => "$args->{self_alias}.remote_ip"}
      };
  },
  { join_type => 'LEFT' },
);
...
Returns the C<portinfo> table entry matching this Port, if any. port_info
provides York specific port information.
